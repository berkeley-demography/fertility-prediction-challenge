...
prep_data_targets <- list(
  ...
  tar_target(
    df_test,
    testing(split_data) %>%
      select(-any_new_child_in_hh, 
             -new_child,
             -nohouse_encr)
  ),
  ## the test set with outcome (for assessing fit)
  tar_target(
    df_test_with_outcome,
    testing(split_data)
  ),
  ## the training set
  tar_target(
...
workflow_set_targets <- list(
  ...
  tar_target(
    best_result_id,
    tune_results %>%
      rank_results('f_meas', select_best = TRUE) %>%
      head(1) %>%
      pull(wflow_id)
  ),
  tar_target(
    best_result,
    tune_results %>% 
      extract_workflow_set_result(id=best_result_id) %>%
      select_best(metric = "f_meas") 
  ),
  tar_target(
    best_final_fit,
    tune_results %>%
      extract_workflow(id=best_result_id) %>%
  ...
  tar_target(
    save_final_fit,
    saveRDS(bundle(best_final_fit %>% extract_workflow()), 
            file.path(prepped_data_dir, 'best_final_fit.rds'))
  )
)
  
#################################
## targets related to ensembles
ensemble_targets <- list(
  tar_target(
    stacked_models,
    stacks() %>%
  ...
  tar_target(
    ensemble_results,
    blend_predictions(stacked_models,
                      metric = metric_set(f_meas))
  ),
  tar_target(
    ensemble_fit,
    fit_members(ensemble_results)
  ),
  tar_target(
    ensemble_test_pred,
    predict(ensemble_fit, new_data = df_test) %>%
    bind_cols(df_test_with_outcome %>% select(new_child))
  ),
  tar_target(
  ...
  tar_target(
    ...
    ensemble_test_pred %>%
      ensemble_test_metrics(truth = new_child, 
                            estimate = .pred_class)
  ),
  ## TODO - can't get ensemble to save for some reason?
  tar_target(
    bundled_ensemble_fit,
    bundle(ensemble_fit %>% butcher())
  ),
  tar_target(
    save_ensemble_fit,
    saveRDS(bundled_ensemble_fit, 
            file.path(prepped_data_dir, 'best_ensemble_fit.rds'))
  )  
  # TODO - should also evaluate this ensemble on the
  # test set
)
...
list(
  ...
  # not running ensemble targets for now
  #ensemble_targets,
  # not running notebook targets for now
  #notebook_targets
)
